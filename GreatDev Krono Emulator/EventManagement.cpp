...
void CEventManagement::Run()
{
	...
	if ( this->m_wToday_Year != sysTime.wYear || this->m_wToday_Month != sysTime.wMonth || this->m_wToday_Day != sysTime.wDay )
	{
		...
		LogAddTD("¡Û¡Ü[Event Management] Date changed. %02d %02d %02d", this->m_wToday_Year, this->m_wToday_Month, this->m_wToday_Day);	// Deathway Need Translation
	...
	for ( it = this->m_vtEventTime.begin() ; it != this->m_vtEventTime.end() ; it++ )
	{
		...
		{
			LogAddTD("¡Û¡Ü[Event Management] Event started. (%d) %02d %02d (state=%d)",
